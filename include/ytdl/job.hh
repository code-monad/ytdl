...
namespace ytdl {
    ...
    struct job : public basic_job { // a job that could be execute  lately
        ...
        job(std::function<Fn>& fn, Args... args) : _functor {[this, fn, args...]{ _result = fn(args...);}} {
    ...
    struct pure_job : public basic_job { // A pure_job is some jobs without any return value
        std::function<void()> _functor;
        pure_job(std::function<Fn>& fn , Args... args) : _functor {[this, fn, args...]{ fn(args...);}}{ }
        void execute() override { _functor(); }
    ...
    auto to_job(std::function<Fn> fn, Args... args) {
        if constexpr(std::is_same_v<typename std::function<Fn>::result_type, void>) return pure_job(fn, args...);
        else return job(fn, args...);
    }
